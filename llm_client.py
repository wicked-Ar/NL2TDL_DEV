...
4. constraints: 제약 조건 리스트 (예: slow, fast, careful, precise 등)
5. task_description: 작업에 대한 간단한 설명 (1-2 문장)
...
대상 물체: {', '.join(objects) if objects else '없음'}
위치: {', '.join(locations) if locations else '없음'}
제약 조건: {', '.join(constraints) if constraints else '없음'}
...
4. Transfer 동작 시퀀스 (pick-and-place):
```
GOAL Execute_Process()
{{
...
완전하고 실행 가능한 TDL 문서를 생성하세요. 표준 TDL 명세를 정확히 따르세요. 다른 설명 없이 TDL 문서만 출력하세요."""
        ...
        response = self.generate_content(prompt, temperature=0.5, max_tokens=3000)
